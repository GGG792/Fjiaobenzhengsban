...
for _, v in getgc(true) do
    if type(v) == "table" then
        ...
        if s and r and type(r) == "table" and r.send then spk = r; break end
    end
end
...
local function createBeam(orig, targ)
    ...
    if not o or not t or not h or h.Health <= 0 then f:Destroy() return end
    ...
    table.insert(con, rns.RenderStepped:Connect(function()
        if not o.Parent or not t.Parent or h.Health <= 0 then
            for _,c in ipairs(con) do c:Disconnect() end
...
local function attack(target)
    ...
    for _,c in ipairs(hp:GetChildren()) do
        if c:IsA("BasePart") then mb = c; break end
    end
    ...
    spk.send({
        packet = {
            weaponModel = wp,
            relativeOffset = head.CFrame:Inverse() * CFrame.new(head.Position),
            identifier = math.floor(tick()*100),
            hitDirection = dr,
            hitNormal = -dr,
            instance = head,
            hitPart = mb,
            timeStamp = ws:GetServerTimeNow(),
            tick = tick(),
            attackInfo = {comboIndex=ci, attackIndex=ai}
        },
        parried = false
    })
...
task.spawn(function()
    while true do
        task.wait(0.12)
        if not runKillAura then continue end
        local tar = getTarget()
        if tar then
            attack(tar)
        end
