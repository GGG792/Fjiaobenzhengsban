...
local function getZombiesDetailed()
    ...
    if not folder then return zombies end
...
local function getWeaponName()
    ...
    if folder then
        for _, child in ipairs(folder:GetChildren()) do
            if child:IsA("Model") or child:IsA("Tool") then return child.Name end
        end
    end
    return "Pistol"
end
...
local function getNearestCrystal(playerPos)
    ...
    if not voidShards then return nil end
...
local function startLockY(targetCF)
    if lockYConnection then lockYConnection:Disconnect() end
...
local function stopLockY()
    if lockYConnection then lockYConnection:Disconnect(); lockYConnection = nil end
    local char = lp.Character
    if char then
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
    ...
    while killActive do
        ...
        if root then
            ...
            for _, z in ipairs(zombies) do
                ...
                pcall(function() gunHitRemote:FireServer(weapon, z.id, z.position) end)
                task.wait(0.02)
            end
        end
        task.wait(0.05)
...
TabBasic:Toggle({
    ...
    Callback = function(state)
        ...
        if hum then hum.WalkSpeed = state and currentSpeed or 16 end
    end
})
...
TabBasic:Slider({
    ...
    Callback = function(value)
        currentSpeed = value
        local hum = lp.Character and lp.Character:FindFirstChild("Humanoid")
        if speedEnabled and hum then hum.WalkSpeed = value end
    end
})
...
TabBasic:Toggle({
    ...
    Callback = function(state)
        noclipEnabled = state
        if noclipCoroutine then task.cancel(noclipCoroutine) end
        if state then
...
TabFeature:Toggle({
    ...
    Callback = function(state)
        killActive = state
        if killCoroutine then task.cancel(killCoroutine) end
        if state then killCoroutine = task.spawn(killLoopFunc) end
    end
})
...
TabFeature:Toggle({
    ...
    Callback = function(state)
        ...
        if autoBuyHealthCoroutine then task.cancel(autoBuyHealthCoroutine) end
        if state then
            autoBuyHealthCoroutine = task.spawn(function()
                while autoBuyHealthEnabled do
                    ...
                    if hum and hum.Health < 30 and tick() - lastHealthPurchaseTime >= 2 then
                        ...
                        if purchaseHealthRemote then pcall(function() purchaseHealthRemote:FireServer() end) end
                    end
                    task.wait(0.5)
                end
            end)
        ...
        if state then
            ...
            autoUpgradeWeaponCoroutine = task.spawn(function()
                while autoUpgradeWeaponEnabled do
                    local success, text = pcall(function() return lp.PlayerGui.MainGui.ControlPanel2.NextUpgrade.TextLabel.Text end)
                    if success and text then
                        local num = tonumber(text:match("%d+")) or 0
                        for i, threshold in ipairs(upgradeLevels) do
                            if not upgradeSent[i] and num > threshold then
                                if purchaseWeaponRemote then pcall(function() purchaseWeaponRemote:FireServer() end) end
                                upgradeSent[i] = true
                            end
                        end
                    end
...
TabAuto:Toggle({
    ...
    Callback = function(state)
        ...
        if autoSkipRoundCoroutine then task.cancel(autoSkipRoundCoroutine) end
        if state then
...
TabAuto:Toggle({
    ...
    Callback = function(state)
        ...
        if autoPickupCoroutine then task.cancel(autoPickupCoroutine) end
        if state then
