...
task.spawn(function()
    while true do
        if _G_StepLoop then
            for i = 1, 10 do StepEvent:FireServer() end
        end
        task.wait(0.1)
    end
end)
...
task.spawn(function()
    while true do
        if _G_AutoRebirth then RebirthEvent:FireServer() end
        task.wait(0.5)
    end
end)
...
CombatTab:Toggle({
    Title = "自动刷经验",
    Value = false,
    Callback = function(state) _G_StepLoop = state end
})

CombatTab:Toggle({
    Title = "自动重生",
    Value = false,
    Callback = function(state) _G_AutoRebirth = state end
})
...
CombatTab:Toggle({
    ...
    Callback = function(state) 
        ...
        WindUI:Notify({
            Title = "传送系统",
            Content = state and "已开始循环传送" or "已停止传送",
            Duration = 2
        })
    end
})
