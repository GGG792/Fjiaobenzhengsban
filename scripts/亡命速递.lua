...
if not inLobby then
    ...
    local itemEspTypes = Tabs.Esp:Dropdown({
        Title = "显示类型",
        Values = {"追踪线", "名称", "距离", "价格"},
        Value = getgenv().Aero.ItemEspTypes,
        Multi = true,
        AllowNone = false,
    ...
    local itemEspToggle = Tabs.Esp:Toggle({
        Title = "启用物品透视",
        Default = getgenv().Aero.ItemEspEnabled,
    ...
    local itemPriceInput = Tabs.Esp:Input({
        Title = "最低价格",
        Value = getgenv().Aero.ItemEspMinPrice,
    ...
    local monsterEspTypes = Tabs.Esp:Dropdown({
        Title = "显示类型",
        Values = {"方框", "名称", "距离"},
        Value = getgenv().Aero.MonsterEspTypes,
        Multi = true,
        AllowNone = false,
        Callback = function(selectedValues)
    ...
    local monsterEspToggle = Tabs.Esp:Toggle({
        Title = "启用怪物透视",
        Default = getgenv().Aero.MonsterEspEnabled,
        Callback = function(state)
    ...
    local npcEspTypes = Tabs.Esp:Dropdown({
        Title = "显示类型",
        Values = {"方框", "名称", "距离"},
        Value = getgenv().Aero.NpcEspTypes,
        Multi = true,
        AllowNone = false,
        Callback = function(selectedValues)
    ...
    local npcEspToggle = Tabs.Esp:Toggle({
        Title = "启用NPC透视",
        Default = getgenv().Aero.NpcEspEnabled,
        Callback = function(state)
...
do
    ...
    local speedToggle = Tabs.Main:Toggle({
        Title = "速度加快",
        Default = getgenv().Aero.SpeedEnabled,
        Callback = function(state)
            if inLobby then
...
do
    ...
    local staminaToggle = Tabs.Main:Toggle({
        Title = "无限体力",
        Default = getgenv().Aero.StaminaEnabled,
        Callback = function(state)
            if inLobby then
                Library:Notify({ Title = "提示", Content = "大厅中无法使用此功能", Duration = 2 })
                return
...
do
    ...
    local invincibleToggle = Tabs.Main:Toggle({
        Title = "无敌",
        Default = getgenv().Aero.InvincibleEnabled,
        Callback = function(state)
            if inLobby then
                Library:Notify({ Title = "提示", Content = "大厅中无法使用此功能", Duration = 2 })
                return
...
local anySellToggle = Tabs.Main:Toggle({
    Title = "随处出售",
    Default = getgenv().Aero.AnySellEnabled,
    Callback = function(state)
        if inLobby then
            Library:Notify({ Title = "提示", Content = "大厅中无法使用此功能", Duration = 2 })
            return
...
local fullBrightToggle = Tabs.Main:Toggle({
    Title = "全亮",
    Default = getgenv().Aero.FullBrightEnabled,
    Callback = function(state)
        if inLobby then
            Library:Notify({ Title = "提示", Content = "大厅中无法使用此功能", Duration = 2 })
            return
...
if not inLobby then
    local autoCollectToggle = Tabs.Auto:Toggle({
        Title = "自动收集",
        Default = getgenv().Aero.AutoCollectEnabled,
        Callback = function(state)
...
if not inLobby then
    local autoOpenToggle = Tabs.Auto:Toggle({
        Title = "自动打开",
        Default = getgenv().Aero.AutoOpenEnabled,
        Callback = function(state)
...
if not inLobby then
    local autoNextFloorToggle = Tabs.Auto:Toggle({
        Title = "自动下一层",
        Default = getgenv().Aero.AutoNextFloorEnabled,
        Callback = function(state)
...
if not inLobby then
    local autoPickupToggle = Tabs.Auto:Toggle({
        Title = "自动拾取",
        Default = getgenv().Aero.AutoPickupEnabled,
        Callback = function(state)
...
if not inLobby then
    local autoCollectEggToggle = Tabs.Auto:Toggle({
        Title = "自动收集蛋",
        Default = getgenv().Aero.AutoCollectEggEnabled,
        Callback = function(state)
...
local autoStartToggle = Tabs.Auto:Toggle({
    Title = "自动开局",
    Default = getgenv().Aero.AutoStartEnabled,
    Callback = function(state)
...
if not inLobby then
    local autoStopToggle = Tabs.Auto:Toggle({
        Title = "自动终止",
        Default = getgenv().Aero.AutoStopEnabled,
        Callback = function(state)
...
if hasFileSystem then
    pcall(function()
        if not isfolder("Aero") then makefolder("Aero") end
    ...
    if success then
        ...
        for flag, control in pairs(_G.AeroControls) do
            if control.Set and getgenv().Aero[flag] ~= nil then
                pcall(function() control:Set(getgenv().Aero[flag]) end)
            elseif control.Select and getgenv().Aero[flag] ~= nil then
                pcall(function() control:Select(getgenv().Aero[flag]) end)
            elseif control.SetValue and getgenv().Aero[flag] ~= nil then
                pcall(function() control:SetValue(getgenv().Aero[flag]) end)
            end
        end
        return true
    end
