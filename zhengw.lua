...
if (LocalPlayer.Name == AUTHOR_NAME) then
	currentRole = "author";
elseif (VIP_USERS[LocalPlayer.UserId] or VIP_USERS[LocalPlayer.Name]) then
	currentRole = "vip";
end
pcall(function()
	if CoreGui:FindFirstChild("FScriptHub") then
		CoreGui.FScriptHub:Destroy();
	end
	if LocalPlayer.PlayerGui:FindFirstChild("FScriptHub") then
		LocalPlayer.PlayerGui.FScriptHub:Destroy();
	end
end);
...
local Window = ChronixUI:CreateWindow({Name="F脚本中心 v6.0",ShowLoadingAnimation=true,CloseCallback=function()
end});
local loadedModules = {};
local buttonStates = {};
local function loadModule(moduleName)
	...
	if success then
		...
		ChronixUI:Notify({Title="模块加载",Content=(moduleName .. " 加载成功"),Type="success",Duration=2});
		return result;
	else
		ChronixUI:Notify({Title="模块加载失败",Content=(moduleName .. " 加载失败: " .. tostring(result):sub(1, 50)),Type="error",Duration=3});
		return nil;
	end
end
local PlayerTab = Window:CreateTab({Name="玩家功能",HasIcon=false});
PlayerTab:AddToggle({Label="启用飞行",Default=false,Callback=function(Value)
	local mod = loadModule("Fly");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="旋转脚本",Default=false,Callback=function(Value)
	local mod = loadModule("Spin");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="环绕旋转",Default=false,Callback=function(Value)
	local mod = loadModule("Orbit");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="无头效果",Default=false,Callback=function(Value)
	local mod = loadModule("Headless");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="燃烧效果",Default=false,Callback=function(Value)
	local mod = loadModule("FireEffect");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="烟雾效果",Default=false,Callback=function(Value)
	local mod = loadModule("SmokeEffect");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="加速脚本",Default=false,Callback=function(Value)
	local mod = loadModule("Speed");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="跳跃增强",Default=false,Callback=function(Value)
	local mod = loadModule("Jump");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="穿墙脚本",Default=false,Callback=function(Value)
	local mod = loadModule("Noclip");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="ESP透视",Default=false,Callback=function(Value)
	local mod = loadModule("ESP");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="无限连跳",Default=false,Callback=function(Value)
	local mod = loadModule("InfiniteJump");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="点击传送",Default=false,Callback=function(Value)
	local mod = loadModule("ClickTP");
	if mod then
		mod.toggle();
	end
end});
PlayerTab:AddToggle({Label="自由相机",Default=false,Callback=function(Value)
...
ProtectTab:AddToggle({Label="防挂机",Default=false,Callback=function(Value)
	local mod = loadModule("AntiAFK");
	if mod then
		mod.toggle();
	end
end});
ProtectTab:AddToggle({Label="防甩飞",Default=false,Callback=function(Value)
	local mod = loadModule("AntiFling");
	if (mod and mod.toggleAntiFling) then
		mod.toggleAntiFling();
	end
end});
ProtectTab:AddToggle({Label="防踢出",Default=false,Callback=function(Value)
	local mod = loadModule("AntiFling");
	if (mod and mod.toggleAntiKick) then
		mod.toggleAntiKick();
	end
end});
local VisualTab = Window:CreateTab({Name="视觉功能",HasIcon=false});
VisualTab:AddToggle({Label="夜视模式",Default=false,Callback=function(Value)
	local mod = loadModule("NightVision");
	if (mod and mod.toggleNightVision) then
		mod.toggleNightVision();
	end
end});
VisualTab:AddToggle({Label="X光透视",Default=false,Callback=function(Value)
	local mod = loadModule("NightVision");
	if (mod and mod.toggleXRay) then
		mod.toggleXRay();
	end
end});
local VehicleTab = Window:CreateTab({Name="载具功能",HasIcon=false});
VehicleTab:AddToggle({Label="车辆加速",Default=false,Callback=function(Value)
...
ToolTab:AddToggle({Label="快速互动",Default=false,Callback=function(Value)
	local mod = loadModule("InstantAction");
	if mod then
		mod.toggle();
	end
end});
ToolTab:AddButton({Text="添加路径点",Callback=function()
	local mod = loadModule("Waypoint");
	if (mod and mod.add) then
		mod.add("路径点 " .. os.time());
		ChronixUI:Notify({Title="路径点",Content="已添加当前位置",Type="success",Duration=2});
	end
end});
ToolTab:AddButton({Text="传送到路径点",Callback=function()
	local mod = loadModule("Waypoint");
...
SettingsTab:AddButton({Text="清理其他脚本UI",Callback=function()
	...
	ChronixUI:Notify({Title="清理完成",Content=("已清理 " .. clearedCount .. " 个脚本UI"),Type="info",Duration=3});
end});
SettingsTab:AddButton({Text="加载 Infinite Yield",Callback=function()
	pcall(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source"))();
	end);
	ChronixUI:Notify({Title="管理员工具",Content="Infinite Yield 已加载",Type="success",Duration=3});
end});
SettingsTab:AddButton({Text="4:3 比例 (FOV 70)",Callback=function()
	Camera.FieldOfView = 70;
	ChronixUI:Notify({Title="4:3比例",Content="FOV已设为70",Type="info",Duration=2});
end});
SettingsTab:AddButton({Text="超广角 (FOV 120)",Callback=function()
	Camera.FieldOfView = 120;
	ChronixUI:Notify({Title="超广角",Content="FOV已设为120",Type="info",Duration=2});
end});
ChronixUI:Notify({Title="F脚本中心",Content=("v6.0 已加载 | 使用 ChronixHub UI 库 | 共 " .. #scripts .. " 个服务器脚本"),Type="success",Duration=5});
